...
const BattleComplete: React.FC = () => {
  ...
  const tweetHref = useMemo(() => {
    ...
    `My ${data.winningItem.metadata.name} just beat a ${data.losingItem.metadata.name} in a ${playoffVerbForTx(txHash as string)}\n\nLet's play. https://sps.calypsohub.network`
  ...
  return (
    <Layout>
      ...
      {data.playerIsWinner && data.tierUnlocked && <Heading>Tier Unlocked!</Heading>}
      {data.playerIsWinner && (
